...
def state_play(value):
    ...
    while 1:
    	...
    	for e in pygame.event.get():
    		...
    		if e.type == KEYDOWN and e.key in (K_q,K_ESCAPE):
    			quit = 1
    		if e.type == KEYDOWN and e.key in (K_SPACE,K_RETURN,K_p):
    			pause = 1
    		if e.type == KEYDOWN and e.key in (K_LCTRL,K_RCTRL,K_LALT,K_RALT,K_LSHIFT,K_RSHIFT):
...
def getch():
    while 1:
    	for e in pygame.event.get():
    		if e.type == KEYDOWN and e.key in (K_SPACE,K_RETURN,K_ESCAPE,K_y,K_n): return e.key
...
def state_init(value):
    ...
    screen = pygame.display.set_mode((320,240),MYSURFACE|SCALED)
    #screen = pygame.display.set_mode((320,240),MYSURFACE)
